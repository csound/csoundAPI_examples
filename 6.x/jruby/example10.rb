...
class ChannelUpdater
  def initialize(csound, channelName, updater)
    @updater = updater
...
channels = [
  ChannelUpdater.new(c, "amp", RandomLine.new(0.4, 0.2)),
  ChannelUpdater.new(c, "freq", RandomLine.new(400, 80)),
  ChannelUpdater.new(c, "resonance", RandomLine.new(0.4, 0.3))
]

# Initialize all Channel Values
#channels.each { |chn| chn.update }
channels.map(&:update)
